...
- [ ] 无 `@keyframes popIn`
- [ ] `animate-pulse` 保留（仅 skeleton）
- [ ] 全局背景色 `bg-base-200`（DaisyUI 语义色）
...
- [ ] 音高标签 `w-10 h-10 rounded-full bg-primary/10 text-primary`
...
- [ ] 钢琴键盘桌面 `sticky bottom-0`，手机 `fixed bottom-0 left-0 right-0 z-10 bg-base-100 border-t border-base-300`
- [ ] 白键 `bg-base-100`，黑键 `bg-base-content`，高亮 `bg-primary`
- [ ] VexFlow 实例缓存，仅 MIDI 变化时重绘
- [ ] VexFlow 鼠标滚轮缩放 + 拖拽平移
- [ ] VexFlow 自动适应容器宽度
- [ ] VexFlow DaisyUI 语义色自动跟随主题
- [ ] 桌面 ≥ 1024px：左右分栏
...
- [ ] 正确反馈 `bg-success/10 border border-success/30`
- [ ] 错误反馈 `bg-error/10 border border-error/30`
...
- [ ] 音阶 `w-8 h-8 rounded-full bg-base-200`
...
- [ ] ColorPicker: `input type="color" w-8 h-8 rounded`
- [ ] TextInput: `input input-bordered input-sm`
- [ ] Section: `mb-6` + 标题 `text-sm font-medium text-base-content/60 mb-3`
...
- [ ] SettingsModal.vue: `modal modal-middle`
