...
import { containsInterval } from "@/helpers/chords";
...
export const DEGREE_NAMES = DEGREE_NAME_KEYS.map(
  (key) => i18n.global.t(`circleOfFifths.degreeNames.${key}`) as string,
);
...
export const MODE_NAMES = MODE_NAME_KEYS.map(
  (key) => i18n.global.t(`circleOfFifths.modeNames.${key}`) as string,
);
