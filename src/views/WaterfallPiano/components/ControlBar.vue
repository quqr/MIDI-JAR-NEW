    ...
    <div class="flex items-center gap-3">
      ...
      <div class="flex items-center gap-1">
        ...
        <button
          ...
          :disabled="!hasContent"
          :aria-label="isPlaying ? t('waterfallPiano.pause') : t('waterfallPiano.play')"
      ...
      <!-- 进度条（所有回放都显示） -->
      <div v-if="hasContent" class="flex items-center gap-2 flex-1 min-w-[120px]">
        <span class="text-xs opacity-50 font-mono w-10 text-right">{{ formatTime(currentTime) }}</span>
        <input
          type="range"
          ...
          @input="onSeek"
        />
        <span class="text-xs opacity-50 font-mono w-10">{{ formatTime(duration) }}</span>
      </div>
      ...
      <!-- 已录制音符数 -->
      <div v-if="noteCount > 0 && !midiFileName" class="badge badge-success badge-sm">
        {{ noteCount }} notes
      </div>
...
const hasContent = computed(() => noteCount.value > 0 || midiFileName.value !== "");
