/**
 * 视频导出工具：编码能力探测、分辨率计算与码率估算
 *
 * 录制方案：离线渲染（快于实时，ADR 0016）——OffscreenCanvas 逐帧
 * 复用 drawScoreFrame 绘制 → WebCodecs VideoEncoder 编码 →
 * Mediabunny 封装为 MP4 容器。本模块只做能力探测与参数计算，
 * 逐帧编排见 videoEncoder.ts。
 */
