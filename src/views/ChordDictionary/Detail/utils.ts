...
export function getSubsetChords(
  ...
  const subset = ChordType.all()
    .filter((chordType) => {
      ...
      return (
        chord.setNum !== chordType.setNum &&
        (chord.setNum & chordType.setNum) === chordType.setNum
      );
    })
    .map((chordType) =>
      Chord.getChord(chordType.aliases[0], chord.tonic as string),
    );
...
export function getSupersetChords(
  ...
  const superset = (
    ...
    .filter((chordType) => {
      ...
      return (
        chord.setNum !== chordType.setNum &&
        (chord.setNum & chordType.setNum) === chord.setNum
      );
    })
    .map((chordType) =>
      Chord.getChord(chordType.aliases[0], chord.tonic as string),
