...
function sortItemsFn(a: ChordGroup | ChordItem, b: ChordGroup | ChordItem) {
  if (a.type === "group" && b.type === "group") {
    return SORT_GROUP_NAMES.indexOf(a.value) < SORT_GROUP_NAMES.indexOf(b.value)
      ? -1
      : 1;
  }
...
export function groupObjectToChordGroup(
  ...
  const items = Object.keys(groups).reduce(
    (i, key: string) => {
      const group = groupObjectToChordGroup(
        (groups as GroupObject)[key],
        disabled,
        key,
      );
...
function buildGroupObject(
  ...
  (obj as GroupObject)[key] = buildGroupObject(
    (obj as GroupObject)[key],
    nextPath,
    value,
  );
